...
namespace Market {
    ...
    double YieldCurve::discount(const double t) const {
    ...
    double YieldCurve::forward_rate(const double t1, const double t2) const {
        ...
        if (t2 <= t1)
            throw std::invalid_argument("t2 should be greater than t1");
        const double fw = std::log(discount(t2)/discount(t1))*(1/(t2-t1));
        return  fw ;
    }


    std::string YieldCurve::get_currency() const {
        return m_ccy;
    }
